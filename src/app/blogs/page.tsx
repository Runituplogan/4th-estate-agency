...
export default function BlogPage() {
  ...
  const filteredPosts = blogPosts.filter((post) => {
    ...
    const matchesCategory =
      activeCategory === "View all" || activeCategory === "Blog categories"
        ? true
          ...
          <div className="grid md:grid-cols-8 grid-cols-2 mt-8 text-xs md:text-sm gap-3 place-content-center text-center">
            {categories.map((item, index) => (
              <p
                ...
                className={`px-3 py-1 cursor-pointer font-geist rounded transition-colors ${
                  activeCategory === item
        ...
        <div className="my-14 animate-fade-up">
          <Wrapper>
            {currentPosts.length > 0 ? (
              ...
              <div className="text-center py-20 text-gray-400 font-geist text-lg">
                No articles found.
              </div>
            )}
